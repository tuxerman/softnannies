...
#fix the default pipeage
def DefaultPipeage(access_mode)

    #figure out if all packets should be dropped or forwarded onwards to a default pipe/behavior
    ...
    #drop packets if so desired 
    if(access_m['1'] == 0):
	return drop

    #set up forwarding ports for the default pipe
    self.access_d = none
    ...
    return if_(match(switch=up_switch, inport=int(self.access_d.port_up)), fwd(port_wan),
			    if_(match(switch=self.up_switch, inport=port_wan), fwd(int(self.access_d.port_up)),
			        if_(match(switch=self.down_switch, inport=int(self.access_d.port_down)), identity, fwd(int(self.access_d.port_down)))))
 

def PolicyManager(access_c, DeviceID, Max):
    policy = access_c[DeviceID]
    if DeviceID-1 == Max:
        return DefaultPipeage() 
